document.addEventListener("DOMContentLoaded", () => {
  ...
  window.unregisterParticipant = async (activityName, email) => {
    try {
      ...
      if (response.ok) {
        showMessage("Successfully unregistered!", "success");
        await fetchActivities(); // Refresh UI instantly without hard reload
      } else {
        showMessage(result.detail || "Could not unregister student.", "error");
      }
    } catch (error) {
  ...
  async function fetchActivities() {
    try {
      ...
      activities.forEach((activity) => {
        ...
        // Generate the HTML using the clean CSS classes found in styles.css
        let participantsHtml = "";
        if (activity.participants && activity.participants.length > 0) {
          participantsHtml = `
          ...
          participantsHtml = `
            <div class="participants-section">
              <em style="color: #777; font-size: 0.9em;">No students enrolled yet.</em>
            </div>
          `;
        }
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        showMessage(result.message, "success");
        signupForm.reset();
        await fetchActivities(); // Updates layout dynamically
      } else {
        showMessage(result.detail || "An error occurred while signing up.", "error");
      }
    } catch (error) {
